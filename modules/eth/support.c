...
static void cpsw_set_promiscious(struct net_device* ndev, bool enable) {
    ...
    for (i = 0; i < cpsw->data.slaves; i++)
        if (cpsw->slaves[i].ndev && (cpsw->slaves[i].ndev->flags & IFF_PROMISC))
            enable_uni = true;

    if (!enable && enable_uni) {
        enable = enable_uni;
        dev_dbg(cpsw->dev,
                "promiscuity not disabled as the other interface is still in "
                "promiscuity mode\n");
    }
...
static int cpsw_ndo_get_phys_port_name(struct net_device* ndev,
                                       char* name,
                                       size_t len) {
    struct cpsw_priv* priv = netdev_priv(ndev);
    int err;
...
static int cpsw_ndo_xdp_xmit(struct net_device* ndev,
                             int n,
...
static void cpsw_port_offload_fwd_mark_update(struct cpsw_common* cpsw) {
    ...
    if (!cpsw->ale_bypass && (cpsw->br_members == (ALE_PORT_1 | ALE_PORT_2)))
        set_val = 1;
...
static int cpsw_netdevice_port_link(struct net_device* ndev,
    ...
    err = switchdev_bridge_port_offload(ndev, ndev, NULL, NULL, NULL, false,
                                        extack);
    if (err)
        return err;
    ...
    switch (event) {
        case NETDEV_CHANGEUPPER:
            ...
            if (netif_is_bridge_master(info->upper_dev)) {
                if (info->linking)
                    ret =
                        cpsw_netdevice_port_link(ndev, info->upper_dev, extack);
                else
...
static int cpsw_dl_switch_mode_get(struct devlink* dl,
                                   u32 id,
...
static int cpsw_dl_switch_mode_set(struct devlink* dl,
                                   u32 id,
                                   struct devlink_param_gset_ctx* ctx) {
    struct cpsw_devlink* dl_priv = devlink_priv(dl);
    struct cpsw_common* cpsw = dl_priv->cpsw;
...
static int cpsw_dl_ale_ctrl_get(struct devlink* dl,
                                u32 id,
                                struct devlink_param_gset_ctx* ctx) {
    struct cpsw_devlink* dl_priv = devlink_priv(dl);
    struct cpsw_common* cpsw = dl_priv->cpsw;
...
static int cpsw_dl_ale_ctrl_set(struct devlink* dl,
                                u32 id,
                                struct devlink_param_gset_ctx* ctx) {
    struct cpsw_devlink* dl_priv = devlink_priv(dl);
    struct cpsw_common* cpsw = dl_priv->cpsw;
    ...
    switch (id) {
        case CPSW_DL_PARAM_ALE_BYPASS:
            ret =
                cpsw_ale_control_set(cpsw->ale, 0, ALE_BYPASS, ctx->val.vbool);
            if (!ret) {
